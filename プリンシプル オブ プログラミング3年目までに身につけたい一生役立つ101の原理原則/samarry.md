...
### **6.1 曳光弾（ひこうだん, Tracer Bullet）**

> 「まず動く最小限のものを作る」

* 曳光弾とは、実弾の中に光を発する弾を混ぜ、弾道を目視できるようにする技術。
* プログラミングでは、“本番に近い動くプロトタイプ”を早く作って方向を確かめることを意味する。
* 完成度は低くても、実際に動くものを通じて設計の方向性を確かめる。

🧩 **例**：
APIを設計する前に、モックデータでフロントから動作確認を行う。
→ 実際のデータが来たときの流れを早めに把握できる。
...
* 関数やモジュールの間には“契約”がある。
  ...
  * 出力条件（Postcondition）
  * 不変条件（Invariant）
* これを明示すると、利用者も安心して使える。

🧩 **例**：
「この関数は引数に null を渡してはいけない」「返り値は必ず正の整数」などをドキュメントや型定義で明確にする。
...
* 想定外の入力や例外が起きても、安全に動作させる。
* “バグを防ぐ”より“バグが起きても壊れない”コードを書く。
* 失敗を前提に、例外処理やバリデーションを徹底する。

🧩 **例**：
APIから想定外のデータが来ても落ちないように`try/catch`でガードする。
`undefined`チェックを怠らない。
...
* “自社のドッグフードを食べる”という言葉から。
* 開発者自身がそのプロダクトのユーザーになることで、改善点を実感できる。
* “開発者視点”ではなく“ユーザー視点”で磨く習慣をつくる。

🧩 **例**：
自分たちの作った管理ツールを、実際に社内業務で利用してみる。

---

### **6.5 ラバーダッキング（Rubber Duck Debugging）**

> 「誰かに説明するつもりで話すと、解決策が見える」

* デバッグのとき、他人や人形（ラバーダック）に問題を説明すると、話しているうちに自分で原因に気づく現象。
* “頭の中の混乱”を“言葉に出して整理”することがポイント。

🧩 **例**：
「なぜバグが出るんだろう？」と声に出して説明しているうちに、「あっ、null チェックしてなかった！」と気づく。
...
* 原則やデザインパターンも、万能ではない。
* チームの規模、開発スピード、ドメイン（業務領域）によって最適解は違う。
* “コンテキストを読む力”が、上級エンジニアへの一歩。

🧩 **例**：
スタートアップではスピード重視でYAGNIを徹底、
大企業では保守性重視でOCPを優先する、など。

---



## ⚠️ 第7章：法則 ～プログラミングのアンチパターン～


### **7.1 ブルックスの法則（Brooks’ Law）**

> 「遅れているプロジェクトに人を増やすと、さらに遅れる」

* 新しい人を増やすと、教育コストとコミュニケーションコストが急増する。
* 作業の分割も難しくなり、チーム全体の効率が落ちる。
 * 🧩 **教訓**：問題は「人数」ではなく「構造」にある。仕組みを見直せ。

---

### **7.2 コンウェイの法則（Conway’s Law）**

> 「組織の構造は、そのままシステム構造に反映される」

* チームの分け方・意思疎通の流れが、そのままコードのモジュール構造になる。
* 縦割り組織は縦割りシステムを生む。
 * 🧩 **教訓**：よいアーキテクチャは、よいチーム構造から生まれる。

---

### **7.3 割れた窓の法則（Broken Windows Theory）**

> 「放置された小さな“汚れ”が、やがて全体を壊す」

* 少しの悪いコード（命名・コメント・仮実装）を放置すると、開発者が“どうでもいい”と思い始める。
* そこから全体の品質が雪崩のように下がる。
 * 🧩 **教訓**：小さな修正を惜しむな。きれいなコード文化を守れ。

---

### **7.4 エントロピーの法則（Law of Entropy）**

> 「放っておくと、すべてのコードは劣化する」

* コードベースは時間とともに複雑化し、乱れていく。
* “保守しない限り、品質は自然に落ちる”という現実を受け入れる。
 * 🧩 **教訓**：リファクタリングは贅沢ではなく、**保守の一部**。

---

### **7.5 80-10-10の法則**

> 「プロジェクトの価値の8割は、全体の1割の部分に集中している」

* ユーザーが最も使う部分（体験・速度・安定性）に注力すべき。
* “全部大事”と考えると、全部が中途半端になる。
 * 🧩 **教訓**：優先順位をつけよう。力をかける部分を選べ。

---

### **7.6 ジョシュアツリーの法則（Joshua Tree Law）**

> 「名前をつけることで、問題は見える化される」

* 名前のない問題は意識されず、改善されない。
* バグ・課題・構造的欠陥にも名前を与えることで、チーム全体で共有できる。
 * 🧩 **教訓**：命名は問題解決の第一歩。

---

### **7.7 セカンドシステム症候群（Second-System Effect）**

> 「2作目は、欲張りすぎて失敗する」

* 初代の成功体験から「次はもっと良く」と盛り込みすぎる。
* 結果として複雑で使いづらいシステムになる。
 * 🧩 **教訓**：シンプルを貫け。成功の次こそ原点に戻る。

---

### **7.8 車輪の再発明（Reinventing the Wheel）**

> 「すでにあるものを、わざわざ自作してしまう」

* “学習目的”ならOKだが、実務ではコストとリスクが大きい。
* OSSや標準ライブラリを活用するほうが安全で早い。
 * 🧩 **教訓**：**「自分で作るより、誰がメンテできるか」**を考えよう。

---

### **7.9 ヤクの毛刈り（Yak Shaving）**

> 「本来の目的から脱線して、関係ない作業にハマる」

* ある問題を直そうとして、気づけばツール設定や別の問題に夢中になっている。
* 一見“努力してる感”はあるが、本質的には遠回り。
 * 🧩 **教訓**：「今やっている作業は、最初の目的につながっているか？」を常に確認せよ。
