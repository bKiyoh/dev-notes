...
`watch` は、Vue.js において **リアクティブな値の「変化」を検知し、それを契機として\*1 副作用処理を実行するための仕組み** です。
...
これらはすべて、 **「状態変化に反応した副作用制御」** という同一の課題に集約されます。
...
`watch` を適切に使うことで、以下の効果があります。

- 副作用を computed から分離できる
- 非同期処理の競合を防げる
- DOM 更新タイミングを制御した処理を書ける
- 状態変化に対する責務が明確になる
...
`watch` には、状態変化の「捉え方」や
副作用処理の「実行タイミング」を制御するための
オプションが用意されています。

ここでは、それぞれのオプションが
**どのような制御軸を持ち、どんな場面で使われるのか**
を整理します。
...
つまり、watch は「使えるから使う」ものではなく、設計判断を伴う API です。

---

## watch を使いすぎないための判断軸

そのため、以降では「この処理は watch で書くべきか」を設計として判断するための観点を整理します。

### ① それは「値の派生」か「状態変化への反応」か

- 値を計算して返したい → `computed`
- 値の変化をきっかけに何かしたい → `watch`
...
- ユーザー操作やイベントに対応する → `methods`
- 状態の変化に自動追従すべき → `watch`

👉 **誰がトリガーか分からなくなるなら watch は不適切**

---

### ③ 副作用は「状態と 1 対 1」で結びついているか

- ある状態の変化に対して必ず起こる処理 → `watch`
- 条件・文脈・一時的な理由に依存する → watch にしない

👉 **因果関係が曖昧なら watch にしない**

---

### ④ その watch は「将来説明できる」か

- 「なぜここで watch なのか」を言語化できる → OK
- 「なんとなく反応させたいから」→ 危険

👉 **説明できない watch は負債になりやすい**

---

### ⑤ cleanup や flush が必要になっていないか

- 単純な反応 → OK
- cancel / timing / 同期制御が増えている → 責務過多の兆候

👉 **watch が複雑になり始めたら設計を見直すサイン**
...
ここまで `watch` を中心に、
状態変化に反応する副作用の設計について説明してきました。

補足として、Vue には
依存関係を自動で追跡する
`watchEffect` という API も用意されています。

これは `watch` の制御オプションではなく、
**監視モデルの異なる別の選択肢**です。

### どういうときに使うのが最適か

`watchEffect` は、
複数のリアクティブな状態に依存する副作用を
**その関係性のまま記述したい場合**に適しています。

依存関係を明示的に列挙するよりも、
「この処理はこれらの状態に依存している」
という事実をそのままコードにしたいときに有効です。
...
- 監視対象を明確にしたい → `watch`
- 副作用と状態の関係を簡潔に書きたい → `watchEffect`
- `newValue` / `oldValue` が必要 → `watch`
- 初回実行を避けたい → `watch`
